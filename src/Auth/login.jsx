...
export default function Login() {
  ...
  const onSubmit = async (values, { setSubmitting }) => {
    try {
      ...
      if (localAdmin || localUser) {
        toast.error("Already logged in");
        if (localAdmin) navigate("/dashboard");
        else navigate("/");
        return;
      }

      const adminRes = await api.get(`/admins?email=${values.email}`);
      const admin = adminRes.data?.[0];

      if (admin && admin.role === "admin") {
        if (admin.password !== values.password) throw new Error("Incorrect password");
        const adminData = { id: admin.id, name: admin.name, email: admin.email, role: "admin", isAdmin: true };
      ...
      const userRes = await api.get(`/users?email=${values.email}`);
      ...
      const userData = { id: user.id, name: user.name, email: user.email, role: "user", isAdmin: false };
      loginUser(userData);
